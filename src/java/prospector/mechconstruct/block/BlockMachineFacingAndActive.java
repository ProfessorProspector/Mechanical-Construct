...
public class BlockMachineFacingAndActive extends BlockMachine {
	...
	public BlockMachineFacingAndActive(String name, String tier, boolean noFacing, Class<? extends BlockEntityMachine> entityClass) {
		...
		if (!tier.isEmpty()) {
			...
			ShootingStar.registerModel(new ModelCompound(MechConstruct.MOD_ID, this, "block/machine/" + tier).setInvVariant("active=false,facing=north").setFileName(name));
		} else {
			setRegistryName(MechConstruct.MOD_ID, name);
			ShootingStar.registerModel(new ModelCompound(MechConstruct.MOD_ID, this, "block/machine").setInvVariant("active=false,facing=north").setFileName(name));
		}
	}
