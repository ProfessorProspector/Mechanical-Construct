...
import prospector.mechconstruct.block.BlockMachine;
import prospector.mechconstruct.gui.MechContainer;
import prospector.mechconstruct.gui.blueprint.*;
import prospector.mechconstruct.gui.blueprint.elements.*;
import prospector.mechconstruct.network.MechPacketManager;
import prospector.mechconstruct.network.PacketTankSync;
import prospector.mechconstruct.util.*;
import prospector.mechconstruct.util.slotconfig.DividedIOItemHandler;
import prospector.mechconstruct.util.slotconfig.SidedConfigData;
...
public abstract class BlockEntityMachine extends TileEntity implements ITickable, IBlueprintProvider {
	...
	public MechContainer getContainer(GuiTabBlueprint blueprint, EntityPlayer player) {
		if (hasFluidInventory()) {
			MechPacketManager.networkWrapper.sendToAllAround(new PacketTankSync(getPos(), fluidInventory.serializeNBT()), new NetworkRegistry.TargetPoint(world.provider.getDimension(), pos.getX(), pos.getY(), pos.getZ(), 128));
		}
