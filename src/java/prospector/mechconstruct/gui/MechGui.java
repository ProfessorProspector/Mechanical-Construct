...
import prospector.mechconstruct.gui.blueprint.GuiTabBlueprint;
...
import prospector.mechconstruct.network.PacketGuiTabItemStack;
import prospector.mechconstruct.network.PacketGuiTabMachine;
			...
			button.addPressAction((element, gui, blueprintProvider, mouseX, mouseY) -> {
				if (blueprintProvider.getCurrentTab() != tab) {
					...
					if (gui.blueprint.provider.getProviderType() == IBlueprintProvider.ProviderType.ITEM) {
						MechPacketManager.networkWrapper.sendToServer(new PacketGuiTabItemStack(blueprintProvider.getGuiTabBlueprints().indexOf(tab)));
						...
						MechPacketManager.networkWrapper.sendToServer(new PacketGuiTabMachine(((BlockEntityMachine) blueprintProvider).getPos(), list.indexOf(tab)));
					}
